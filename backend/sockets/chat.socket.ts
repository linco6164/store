  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
        ...
        try {
          ...
          if (user.role !== "admin") {
            socket.emit(
              ...
              {
                message:
                  "Nu ai acces la suportul administrativ.",
              },
            );
          ...
          console.log(
            `[SUPPORT] Admin ${socket.userId} joined support:admins`,
          );
        } catch (error) {
          console.error(
            "[SUPPORT] ADMIN JOIN ERROR:",
            error,
          );
          ...
          socket.emit(
            ...
            {
              message:
                "Nu s-a putut conecta la suport.",
            },
          );
        }
      },
    ...
    socket.on(
      ...
      async () => {
        ...
        console.log(
          `[SUPPORT] ${socket.userId} left support:admins`,
        );
      },
    );
      ...
      async (ticketId: string) => {
        try {
          ...
          socket.emit(
            ...
            {
              message:
                "Nu s-a putut deschide conversația.",
            },
          );
        }
      },
    ...
    socket.on(
      ...
      async (ticketId: string) => {
        try {
          ...
          console.log(
            `[SUPPORT] New ticket ${ticketId} sent to admins`,
          );
        } catch (error) {
          console.error(
            "[SUPPORT] TICKET CREATED ERROR:",
            error,
          );
        }
      },
    ...
    socket.on(
      ...
      async (data: {
        ...
        try {
          ...
          /*
           * Userul deschide/continuă conversația:
           * open
           *
           * Adminul răspunde:
           * pending
          ...
          // -------------------------------------------------
          // TRIMITEM MESAJUL ÎN CAMERA TICKETULUI
          // -------------------------------------------------
          ...
          console.log(
            `[SUPPORT] ${isAdmin ? "ADMIN" : "USER"} ${socket.userId} sent message in ${room}`,
          );
        } catch (error) {
          console.error(
    ...
    socket.on(
      ...
      (ticketId: string) => {
        ...
        console.log(
          `[SUPPORT] ${socket.userId} left ${room}`,
        );
      },
    );
