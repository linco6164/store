...
import {
  ROLE_DEPARTMENTS,
} from "../modules/support/support.constants.js";

import type {
  SupportTicketDepartment,
} from "../modules/support/support.model.js";
...
function joinStaffRooms(
  ...
  const departments =
    ROLE_DEPARTMENTS[
      user.role as keyof typeof ROLE_DEPARTMENTS
    ];
...
export default function registerSupportSocket(
  io: Server,
) {
  io.on(
    "connection",
    async (socket) => {
        ...
        (
          ...
          try {
            ...
            callback?.({
              success: false,
              message:
                "Nu s-a putut accesa departamentul.",
            });
          }
        },
      );
      ...
      socket.on(
        ...
        (reason) => {
          console.log(
            `[SUPPORT SOCKET] ${user.id} disconnected | ${reason}`,
          );
        },
      );
    },
